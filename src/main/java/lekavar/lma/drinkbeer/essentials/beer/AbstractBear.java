...
public abstract class AbstractBear implements IBeer{
    ...
    public void onDrink(World world, ItemStack stack, LivingEntity livingEntity) {
        ...
        List<IFlavor> flavors = new ArrayList<>(); // TODO: retrieveFlavor()
        IFlavor comboFlavor = ComboFlavors.Sample; // TODO: retrieveComboFlavor()
        ...
        for(IFlavor flavor: flavors){
            ...
            healthModifier = flavor.modifyHeal(world,stack,livingEntity,healthModifier);
            drunkLevelModifier = flavor.drunkLevelModifierModify(world,stack,livingEntity,drunkLevelModifier);
        }
        nutrition = comboFlavor.modifyNutrition(world,stack,livingEntity,nutrition);
        healthModifier = comboFlavor.modifyHeal(world,stack,livingEntity,healthModifier);
        drunkLevelModifier = comboFlavor.drunkLevelModifierModify(world,stack,livingEntity,drunkLevelModifier);
        ...
        for(IFlavor flavor: flavors){
            flavor.onDrink(world, stack, livingEntity);
        }
        comboFlavor.onDrink(world, stack, livingEntity);
    }
}
